...
export function getConfigFromEnv(): MySQLConfig {
  ...
  // HTTP server options
  const httpAuthEnabled = process.env.MCP_HTTP_AUTH_ENABLED === 'true';
  const httpApiKey = process.env.MCP_HTTP_API_KEY;
  ...
  const connectionLimit = connectionLimitStr
    ? parseInt(connectionLimitStr, 10)
    : undefined;
  const queueLimit = queueLimitStr ? parseInt(queueLimitStr, 10) : undefined;
  const connectTimeout = connectTimeoutStr
